...
public class BuildHelper {
    ...
    public List<Map<Object, Object>> handlerInfo() throws Exception {
        ...
        for (String table : tableMapping.keySet()) {
            ...
            if (modelName == null || modelName.trim().length() == 0) {
                ...
                for (char c : chars) {
                    ...
                    if ("_".equals(String.valueOf(c))) {
                        ...
                        if (isTrue) {
                            t.append(String.valueOf(c).toUpperCase());
                            isTrue = false;
                        } else {
                            t.append(c);
                        }
            ...
            for (int i = 0; i < chars.length; i++) {
                if (i == 0) {
					requestUrl.append(String.valueOf(chars[i]).toLowerCase());
                } else {
                    requestUrl.append(chars[i]);
                }
            }
		/*	for (char c :chars ) {
            ...
            tableMap.put("requestUrl", requestUrl.toString());
            ...
            if(tableRemark==null || tableRemark.trim().length()==0) {
                //获取表备注
                ResultSet rs1 = databaseMetaData.getTables(null, null, table, new String[]{"TABLE"});
                while (rs1.next()) {
                    tableRemark = rs1.getString("REMARKS");
                }
            }
            ...
            while (rs.next()) {
                ...
                colMap.put("nameUpper", name.toUpperCase());
    ...
    public Map<String, Object> handlerType(String type) {
        ...
        String[] ints = new String[]{"TINYINT", "INTEGER", "INT", "BIT", "BOOLEAN"};
